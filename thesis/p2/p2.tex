...
\todo{Whole graph and condensed counterpart! (Probably after whole profile stuff.)}
...
\begin{figure}[t]
  ...
  \caption{\todo{Hier braucht man unbedingt die expected run times dazu!}A snapshot DAG (HLF, two processors) and its corresponding profile DAG containing much less nodes than the original snaphsot DAG. \emph{Remark:} Snapshots with same profiles have the same expected run time and can thus be merged. }
...
\begin{figure}[th]
  ...
  \end{subfigure}
  \caption{Snapshot DAGs for two different intrees with the same profile length}
