...
\begin{theorem}
  \label{the:profile-dags-exact-size}
...
\begin{proof}
  We compute the number of nodes in the profile DAG for the profile $P^*= \profile{\profileones{r-2},n-r+1,1}$. According to theorem \ref{the:profile-dags-exact-size}, it is exactly
  \begin{equation*}
...
\begin{proof}
  We consider the number of nodes in a profile DAG for a profile of the form $\profile{\profileones{r-2},n-r+1,1}$, given by theorem \ref{the:profile-dags-exact-size}:
  \begin{equation*}
    \sum_{j=1}^{r-2} j \cdot 1 + (r-1)\cdot(n-r+1) = \frac{(r-2)\cdot(r-1)}{2} + (r-1)\cdot(n-r+1)
  \end{equation*}
  ...
  \end{equation*}
  and recognize that this is a parabola having the derivative
  \begin{equation*}
    3+n-2r,
  \end{equation*}
  meaning that its maximum is at $r=0.5n + 1.5$.
\end{proof}
