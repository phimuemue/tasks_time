...
\begin{definition}[Toptask-maximal subtree for a leaf]
  ...
  The \emph{toptask-maximal subtree for a leaf} $t$ is the subtree rooted at the \emph{lowest} task within $p$ that is not $t$ and that does \emph{not} contain more topmost tasks than its predecessor within $p$.\todo{Versteht man das?}
\end{definition}
...
A strategy now might be to resolve HLF-ambiguities as follows:
\begin{itemize}
...
It can be quickly seen that slightly altering the strategy given in section \ref{sec:suboptimal-hlf-can-optimal-subtree-fewest-toptasks} in the sense that we do not concentrate on the number of \emph{topmost tasks} in a maximal subtree, but on the number of \emph{leaves} in a maximal subtree. We adapt the notion of topmost-maximal subtrees in a straightforward manner:

\begin{definition}[Leaf-maximal subtree for a leaf]
  Let $t$ be a leaf of an intree $I$ and let $p=(t, t_1, t_2, t_3, \dots, r)$ be the path from $t$ to the root $r$.

  The \emph{leaf-maximal subtree for a leaf} $t$ is the subtree rooted at the \emph{lowest} task within $p$ that is not $t$ and that does \emph{not} contain more leaves than its predecessor within $p$.\todo{Versteht man das?}
\end{definition}
