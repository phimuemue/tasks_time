...
\begin{figure}[ht]
  ...
  \begin{subfigure}[b]{.45\textwidth}
    ...
    \vfill
    \caption{This intree is optimally scheduled as shown, but in the optimal schedule for the original intree, it has to be scheduled in another way.}
...
However, there are examples where the optimal contains snapshots that do not adhere to this property. Consider e.g. the tree $(0,0,0,1,1,1,2,2,3)$ (figure \ref{fig:hlfnfc-is-not-optimal} compares the optimal schedule to the no-free-paths schedule).
...
We observed that (for three processors) an optimal schedule looks as if it chose certain subtrees and ``filled them up one after another''. This is probably most concise formalized as follows:
...
An alternative formulation of the above conjecture states that at most one task having as well scheduled as non-scheduled predecessors that are leaves. We can only assume this without loss of generality because e.g. the intree $(0,0,0,1,1,2,2,3)$ admits two different schedules (one beginning with tasks 4,5,8 and the other beginning with 4,6,8) that have exactly the same run time (see figure \ref{fig:filling-up-without-loss-of-generality}). The reason is that both schedules result in equivalent snapshots with the same probabilities.
