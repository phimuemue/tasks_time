...
\begin{proof}
  ...
  Of course, the task $y$ finishes at some point in $S_p$, but it finishes \emph{earlier} in $S_{p+1}$ because it has been scheduled from the beginning.\todo{Figure reinhauen!}
  ...
  However, with $p+1$ processors and scheduling strategy $S_{p+1}$, we know that $y$ finishes earlier, thus allowing $z_1,z_2,z_3,\dots$ to be scheduled earlier because $y$ is already processed. This shows that the overall run time for $S_{p+1}$ is less than the overall run time for $S_p$.
\end{proof}
