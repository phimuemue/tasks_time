...
\end{theorem}
  ...
  %   \centering
  %   \begin{tikzpicture}
  %     \begin{scope}[yscale=.4, xscale=.75]
  %       \draw(0,0) rectangle node{$x_1$} +(4,1);
  %       \draw(0,1) rectangle node{$x_2$} +(8,1);
  %       \draw(0,2) rectangle node{$x_3$} +(5,1);
  %       \draw(4,0) rectangle node{$a$} +(3,1);
  %       \draw(7,0) rectangle node{$b$} +(3.5,1);
  %       \draw(8,1) rectangle node{$c$} +(4,1);
  %       \draw(5,2) rectangle node{$y$} +(4.5,1);
  %       \draw(9.5,2) rectangle node{$z_1$} +(1,1);
  %     \end{scope}
  %     \begin{scope}[yscale=.4, xscale=.75, yshift=-5cm]
  %       \draw(0,0) rectangle node{$x_1$} +(4,1);
  %       \draw(0,1) rectangle node{$x_2$} +(8,1);
  %       \draw(0,2) rectangle node{$x_3$} +(5,1);
  %       \draw(4,0) rectangle node{$a$} +(3,1);
  %       \draw(7,0) rectangle node{$b$} +(3.5,1);
  %       \draw(8,1) rectangle node{$c$} +(4,1);
  %       \draw(0,-1) rectangle node{$y$} +(4.5,1);
  %       \draw(5,2) rectangle node{$z_1$} +(1,1);
  %     \end{scope}
  %   \end{tikzpicture}
  %   \caption{Exemplary scheduling diagram for $p$ and $p+1$ processors with respective schedules $S_p$ and $S_{p+1}$.}
  %   \label{fig:exemplary-schedules-sp-sp1}
  % \end{figure}

  % We now consider tasks that are scheduled \emph{after} task $y$ in $S_p$ (we call them $z_1,z_2,z_3,\dots$). These tasks are scheduled after $y$ in $S_p$ because they either require $y$ to be finished or because they could not be scheduled earlier because the $p$ processors were busy.

%   However, with $p+1$ processors and scheduling strategy $S_{p+1}$, we know that $y$ finishes earlier \emph{and} that -- after $y$ finishes in $S_{p+1}$ the $(p+1)$-th processor can be used to process tasks. 

% thus allowing $z_1,z_2,z_3,\dots$ to be scheduled earlier because $y$ is already processed. This shows that the overall run time for $S_{p+1}$ is less than the overall run time for $S_p$.
...
As we can see in table \ref{tab:num-subtrees-size-of-dags}, the number of subtrees is (at least for $n\geq  9$) significantly larger than the number of snapshots in the snapshot DAG for the optimal schedule. However, the (maximal) overall number of snapshots is (for $n\geq 7$) strictly higher than the (maximal) number of subtrees.
...
% However, it is quite difficult to make exact claims here because we have too few numbers to predict anything with high confidence.
...
We did not include a picture showing all intrees with maximally many subtrees, because there are too many of them. That is, if we just consider the largest snapshot DAGs, there are quite few intrees reaching this maximum, but for the number of snapshots there are many.
...
\begin{figure}[th]
  ...
  \begin{subfigure}{.58\textwidth}
    \centering
    \input{p3/00112334451010_optimal.tex}
  ...
  \begin{subfigure}{.3\textwidth}
    \centering
    \input{p3/00112334451010_optimal_merged.tex}
...
\begin{figure}[ht]
  \centering{}
  \input{p3/00112334451010_leaf_important_parts.tex}
...
\begin{center}  
  \begin{tikzpicture}[scale=.4]
    ...
    \foreach \x in {-2,-1,0,1 }{
      \draw[fill=black] (\x,1) circle (0.3);
      \draw (\x,1) -- (0,0);
    };
    \foreach \x in {-3,...,-1 }{
      \draw[fill=black] (\x,2) circle (0.3);
      \draw (\x,2) -- (-2,1);
    };
    \foreach \x in {-2,...,1}{
      \draw[fill=black] (\x,3) circle (0.3);
      \draw (\x,3) -- (-1,2);
    };
...
\begin{proof}
  ...
  The induction basis is the case where we have degenerate intrees with 3 tasks\footnote{We start with 3 tasks since these trees are the only ones that allow adding $z_1$ and $z_2$ on different levels such that both $x$ and $y$ stay ready. For an intree with two tasks (i.e. for the intree denoted by $(0)$), the claim can be seen by simply examining that $2.5 = T\left( (0,0) \right) < T\left( (0,1) \right) = 3$.} (all of them are depicted in figure \ref{fig:p3-lemma-adding-intrees-induction-start} (only the black nodes)).
  ...
  \begin{figure}[t]
    ...
    \begin{tikzpicture}[scale=0.25]
      ...
      \begin{scope}[yshift=-15cm, xshift=-20cm]
        \begin{scope}[xshift=0cm]
          \treefive;
          \draw (-.50,3) -- (-.5,2);
      ...
      \begin{scope}[yshift=-15cm, xshift=-2cm]
        \begin{scope}[xshift=0cm]
          \treesix;
          \draw (-.50,3) -- (-.5,2);
          \draw[fill=white](-.5,3) circle (0.4);
        ...
        \begin{scope}[xshift=5cm]
          \treesix;
          \draw(0,1) -- (1.5,2);
          \draw[fill=white](1.5,2) circle (0.4);
          \node at (0,-2) {3.83};
        \end{scope}
        \node at (7.5,-2) {$>$};
        \begin{scope}[xshift=10cm]
      ...
      \begin{scope}[yshift=-15cm, xshift=16cm]
        \begin{scope}[xshift=0cm]
          \treeseven;
          \draw (0,3) -- (0,4);
  ...
  \item If $x$ is the first task to finish, then the resulting intrees are 
    ...
    We can now apply the induction hypothesis for the intree $I^x$, since this intree contains only $n-1$ tasks: We have an intree with $n-1$ tasks (namely $I^x$) containing two ready tasks $y$ and $x'$ and two tasks $z_1$ and $z_2$ with $level(z_1)>level(z_2)$, $I^x_1 = I^x\cup\left\{ z_1 \right\}$ and $I^x_2 = I^x\cup\left\{ z_2 \right\}$, implying that $T^*_{x',y,z_1}(I^x_1)>T^*_{x',y,z_2}(I^x_{2})$.
  \item If $y$ is the first task to finish, we argue similar to the $x$ case, thereby considering $I^y_{1},I^y_{2}$ and $I^y$ which are all defined analogously. This finally yields the following inequality: $T^*_{x,y',z_1}(I^y_{1}) > T^*_{x,y',z_2}(I^y_{2})$ (where $y'$ denotes the task that is scheduled next after $y$ finishes).
  \end{itemize}
...
\ref{fig:p3-adding-tasks-level-keep-scheduled-same-inequality}.
  ...
  \begin{figure}[t]
    ...
    \begin{tikzpicture}[scale=.4]
      ...
      \begin{scope}[xshift=-2*\abstand, yshift=-11cm]
        \rawtriangle
        \node[draw=black,circle] at (0,6.5) {$z_1$};
        \drawxx;
        \drawy;
      ...
      \begin{scope}[xshift=-\abstand, yshift=-11cm]
        \rawtriangle
        \node[draw=black,circle] at (0,6.5) {$z_1$};
        \drawx;
        \drawyy;
      ...
      \begin{scope}[xshift=0, yshift=-11cm]
        \treetriangle
      ...
      \draw[thick,->](-\abstand,0) -- node[right]{$y$} +(0,-2.8);
      \draw[thick,->](-\abstand-2cm,1) -- node[above left]{$x$} +(-\abstand+4cm,-3.6);
      \draw[thick,->](-\abstand+2cm,1) -- node[above right]{$z_1$} +(\abstand-4cm,-3.6);

      \begin{scope}[xshift=2*\abstand]
        \draw[thick,->](-\abstand,0) -- node[right]{$x$} +(0,-2.8);
        \draw[thick,->](-\abstand-2cm,1) -- node[above left]{$z_2$} +(-\abstand+4cm,-3.6);
        \draw[thick,->](-\abstand+2cm,1) -- node[above right]{$y$} +(\abstand-4cm,-3.6);
      \end{scope}
      
      % legend      
      \draw[decoration=brace,decorate=true](-11,8.25) --node[above]{$I_1=I\cup\left\{ z_1 \right\}$} +(7,0);
      \draw[decoration=brace,decorate=true](  4,8.25) --node[above]{$I_2=I\cup\left\{ z_2 \right\}$} +(7,0);
      \draw[decoration={brace,mirror},decorate=true](  -18,-11.5) --node[below, yshift=-.2cm]{$I^x_{1}$} +(6,0);
      \draw[decoration={brace,mirror},decorate=true](-10.5,-11.5) --node[below, yshift=-.2cm]{$I^y_{1}$} +(6,0);
      \draw[decoration={brace,mirror},decorate=true](   -3,-11.5) --node[below, yshift=-.2cm]{$I$} +(6,0);
      \draw[decoration={brace,mirror},decorate=true](  4.5,-11.5) --node[below, yshift=-.2cm]{$I^x_{2}$} +(6,0);
      \draw[decoration={brace,mirror},decorate=true](   12,-11.5) --node[below, yshift=-.2cm]{$I^y_{2}$} +(6,0);
    \end{tikzpicture}

    \caption{Proof sketch for lemma \ref{lem:p3-adding-tasks-level-keep-scheduled-same-inequality}. By induction hypothesis, we have that $T^*_{x',y,z_1}(I^x_{1}) > T^*_{x',y,z_2}(I^x_{2})$ and $T^*_{x,y',z_1}(I^y_{1}) > T^*_{x,y',z_2}(I^y_{2})$, from which we deduce $T^*_{x,y,z_1}(I_1) > T^*_{x,y,z_2}(I_2)$. Note that $z_2$ can not block $x'$ or $y'$ since $z_1$ didn't block any of the two and we required that adding $z_1$ resp. $z_2$ still results a degenerate tree. Labels on transitions denote which task is the first to finish.}
...
\begin{table}[t]
  ...
  \begin{tabular}[ht]{ccc}
    ...
    26 &  426089  & 456 \\
  \end{tabular}
  \caption{Number of snapshots for degenerate binary trees in the P3 case. The first column shows the number of tasks. ``Overall'' denotes the number of distinct snapshots that are explored if an optimal schedule is constructed by examining all possible schedules. ``HLF'' denotes the number of distinct snapshots for HLF.}
  \label{tab:p3-degenerate-binary-trees-no-snapshots}
\end{table}

On the other hand, the number of snapshots needed by HLF seems to coincide with the sequence \cite{oeisA055803}.
...
\subsection{Parallel chains with less than 28 tasks are optimally scheduled by HLF}
...
Note that conjecture \ref{conj:parallel-chains-complex-conjecture} implies \ref{conj:parallel-chains-hlf-conjecture}. Unfortunately, we were not able to prove them, although they are somewhat intuitive. Moreover, we verified those conjectures for many examples.

%We start out by a lemma that we will use later. This lemma is an analogue variant of a lemma described in \cite{chandyreynoldsshortpaper1975}. While their lemma works for two processors and general intrees, our variant is for three processors, but restricted to parallel chains. Moreover, we split the lemma into two parts since the respective proof techniques rely on different strategies.

% \begin{lemma}
%   \label{lemma:parallel-chains-flatness-three-or-less-tasks}
%   Let $I$ be a parallel chain with at most three ready tasks. Let $x$ and $y$ two ready tasks with $level(x) > level(y)$. Then, $T_{HLF}(I\setminus\{ x \}) < T_{HLF}(I\setminus\{ y \})$, where $T_{HLF}(I)$ denotes the run time that is acchieved if intree $I$ is scheduled according to HLF.
% \end{lemma}

% \begin{proof}
%   Note that a parallel chain is uniquely (up to isomorphisms) defined by its chain lengths. We denote the sequence of chain lengths as a tuple $\chainlengths{l_1,l_2,\dots,l_r}$ meaning that the root has $r$ predecessors, and the respective chain lengths are $l_1,l_2,\dots,l_r$. Since we are interested in parallel chains with three or less ready tasks, we can for now restrict on triples $\chainlengths{a,b,c}$ indicating three chains of length $a$, $b$ resp. $c$. Note that we can w.l.o.g. assume that $a\geq b$.

%   We now assume w.l.o.g. that $x$ is the leaf task of the chain of length $a$ and $y$ is the task at the beginning of the second chain (whose length is given by $b$). Because we assume $level(x) > level(y)$, we can state that $a>b$.
  
%   The claim
%   \begin{equation*}
%     T_{HLF}(I\setminus\{ x \}) < T_{HLF}(I\setminus\{ y \})
%   \end{equation*}
%   can then be reformulated to
%   \begin{equation*}
%     T_{HLF}(\chainlengths{a-1,b,c}) \leq T_{HLF}(\chainlengths{a,b-1,c})
%   \end{equation*}

%   We proof the claim by induction. The base case (the minimal parallel chain with at least three tasks and $x$ and $y$ on different levels is $(0,0,0,1)$) is illustrated and confirmed in figure \ref{fig:parallel-intrees-lemma-induction-start}.
...
%   The induction step is on the number of tasks. Assume that we know the claim for $n-1$ tasks to be true.

%   Then, take a parallel chain containing $n$ described by chain lengths $\chainlengths{a,b,c}$ with $a > b$.

%   The run time for $\chainlengths{a-1,b,c}$ can be computed by
%   \begin{equation*}
%     T_{HLF}(\chainlengths{a-1,b,c}) = 
%     \frac{1}{3} +
%     \frac{1}{3} \cdot 
%     \left[ 
%     T_{HLF}(\chainlengths{a-2,b,c}) +
%     T_{HLF}(\chainlengths{a-1,b-1,c}) + 
%     T_{HLF}(\chainlengths{a-1,b,c-1}) 
%     \right],
%   \end{equation*}
%   while the run time for $\chainlengths{a,b-1,c}$ is given by
%   \begin{equation*}
%     T_{HLF}(\chainlengths{a,b-1,c}) = 
%     \frac{1}{3} +
%     \frac{1}{3} \cdot 
%     \left[ 
%       T_{HLF}(\chainlengths{a-1,b-1,c}) +
%       T_{HLF}(\chainlengths{a,b-2,c}) + 
%       T_{HLF}(\chainlengths{a,b-1,c-1}) 
%     \right].
%   \end{equation*}

%   By the induction hypothesis, we have:
%   \begin{eqnarray*}
%     T_{HLF}(\chainlengths{a-2,b,c}) & < & T_{HLF}(\chainlengths{a-1,b-1,c}) \\
%     T_{HLF}(\chainlengths{a-1,b-1,c}) & < & T_{HLF}(\chainlengths{a,b-2,c}) \\
%     T_{HLF}(\chainlengths{a-1,b,c-1}) & < & T_{HLF}(\chainlengths{a,b-1,c-1})
%   \end{eqnarray*}

%   This can be used to derive our claim:
%   \begin{eqnarray*}
%     T_{HLF}(\chainlengths{a-1,b,c}) & = &
%     \frac{1}{3} +
%     \frac{1}{3} \cdot 
...
%       T_{HLF}(\chainlengths{a,b-1,c-1}) 
%     \right] = \\
%     &=& T_{HLF}\left( \chainlengths{a,b-1,c} \right)
%   \end{eqnarray*}
% \end{proof}

% We now extend lemma \ref{lemma:parallel-chains-flatness-three-or-less-tasks} to work for an arbitrary number of chains resp. ready tasks.

% \begin{lemma}
%   \label{lemma:parallel-chains-flatness}
%   Let $I$ be a parallel chain and $x$ and $y$ two ready tasks with $level(x) > level(y)$. Then, $T_{HLF}(I\setminus\{ x \}) < T_{HLF}(I\setminus\{ y \})$, where $T_{HLF}(I)$ denotes the run time that is acchieved if intree $I$ is scheduled according to HLF.
% \end{lemma}
...
%   We consider the HLF schedules on the intrees $I\setminus \{x\}$ and $I \setminus \{y\}$, with variable names as aforementioned. Note that the intrees $I\setminus \{x\}$ and $I \setminus \{y\}$ -- since we assume that we have at least three leaves -- have at least two common HLF-conform tasks (i.e. tasks that are scheduled by HLF). We call these two tasks $x_1$ and $x_2$. For the intree $\iminus{x}$, we call the third HLF-task $x_3$ and can compute its expected run time (for HLF) by
%   \begin{equation*}
%     T_{HLF}(\iminus{x}) = 
%     \frac{1}{3} +
...
%   The intrees $\iminus{x,x_1}$ and $\iminus{y,x_1}$ have a common supertree, namely $\iminus{x_1}$. By induction ($\iminus{x_1}$ has $n-1$ tasks, $level(x) > level(y)$), we know that the expected HLF run time for $\iminus{x,x_1}$ is less than the expected run time for $\iminus{y,x_1}$. Note that $\iminus{x, x_1}$ has at least as many ready tasks as $\iminus{y,x_1}$. So, if $\iminus{y,x_1}$ has two or less ready tasks, the claim is true, because it has been shown for two processors in \cite{chandyreynoldsshortpaper1975} and we -- moreover -- can possibly use a third processor for $\iminus{x,x_1}$. This consideration can be done for $x_2$ in the same manner. 

%   It remains to compare $T(\iminus{x, x_3})$ and $T(\iminus{y, x'})$. If $\iminus{y,x'}$ has fewer ready tasks than $T(\iminus{x, x_3})$, then we have $T(\iminus{x, x_3}) \leq T(\iminus{y, x'})$ (similar to the case before, it has been shown for two processors by \cite{chandyreynoldsshortpaper1975}, and we additionally can possibly use a third processor for $\iminus{x,x_3}$). We, thus, focus on the remaining cases where both have the same number of ready tasks.
...
\begin{table}[ht]
  \centering
  \subcaptionbox{Number of snapshots with examined by exhaustive search.}{
    \centering
  ...
  \quad
  \subcaptionbox{Number of snapshots examined by HLF.}{
    \centering
    \begin{tabular}[ht]{c|cccc}
      \diagbox{$L$}{$N$} & 3 & 4 & 5 & 6 \\
      \hline
...
\begin{figure}[th]
  \centering
  \subcaptionbox{One possible schedule.}{
    \centering
    \includegraphics{p3/improving_leaf_001133512_v1.pdf}
  }
  \subcaptionbox{Another possible schedule.}{
    \centering
    \includegraphics{p3/improving_leaf_001133512_v2.pdf}
  }
  \begin{subfigure}{.8\textwidth}
    ...
    \begin{tikzpicture}[scale=.2]
      ...
      \draw[](tid0) -- (tid2);
    \end{tikzpicture}
    \caption{This intree occurs in both schedules, but with different sets of scheduled tasks, requiring to HLF to examine both variants if we compute the optimal run time.}
    \label{fig:improving-leaf-duplicate-task}
  \end{subfigure}
  \caption{Two possible schedules for intree $(0,0,1,1,3,3,5,1,2)$ that must be examined by exhaustive search to compute the optimal schedule. These can be used to illustrate why improving exhaustive search for degenerate intrees and parallel chains is not that useful in some cases. There are several snapshots containing a degenerate intree, each of whom must be processed by HLF separately.}
  \label{fig:improving-leaf-not-that-useful-in-some-cases}
\end{figure}

\emph{Remark:} Even if -- at the point of computation -- there are different snapshots containing the same (or an isomorphic) intree, we did not observe one single instance where the \emph{optimal schedule} contained two distinct snapshots with two isomorphic intrees (but different sets of scheduled tasks). This lead us to the following conjecture:
