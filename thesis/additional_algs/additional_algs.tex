...
\item Line \ref{alg:canonical-intree-numbering-line} ensures that the labels of tasks do not become very long. To be precise, if level $i$ has exactly $l_i$ tasks, the labels for level $i$ are at most the numbers 0 to $l_i$. Since each task gets an integer exactly once, the \emph{total} costs for line 9 (i.e. summed up over all iterations) is $O(n)$.
